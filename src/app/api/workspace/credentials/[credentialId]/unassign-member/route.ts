...
// DELETE - Unassign member from credential
export async function DELETE(
  ...
  const logger = createRequestLogger(
    request,
    `/api/workspace/credentials/${params.credentialId}/unassign-member`
  );
  ...
  try {
    ...
    const response = await loggedFetch(
      `${XANO_WORKSPACE_API_BASE}/credentials/${params.credentialId}/unassign-member`,
      {
        method: 'DELETE',
        headers: {
    ...
    if (!response.ok) {
      const errorResponse = {
        error: data.message || 'Failed to unassign member from credential',
